...
if ! is_running; then
    cd "$QS_DIR" || exit 1
    nohup quickshell -p "$MAIN" --no-duplicate >/tmp/hyprclip.log 2>&1 &
    disown
    [ "$1" = "--daemon" ] && exit 0
    # wait until the IPC answers, then open
    for i in $(seq 1 60); do
        quickshell ipc -p "$MAIN" call hyprclip open >/dev/null 2>&1 && exit 0
        sleep 0.05
    done
    exit 1
fi
...
[ "$1" = "--daemon" ] && exit 0
quickshell ipc -p "$MAIN" call hyprclip toggle
